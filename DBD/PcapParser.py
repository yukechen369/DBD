...
#-*- encoding: utf-8 -*-
import datetime, socket, sys, ipaddr, time, dpkt, struct, getopt
...
class CSVWrapper():
    ...
    def close(self):
        if self.pipeline:
            self.h.find_anomaly()
            self.h.start_console()
            return
        self.req_outfile.close()
        self.res_outfile.close()
        self.log_outfile.close()
        t1 = Thread(target=map_analyse_data, args=(self.filename, 2))
        t1.start()
        t1.join()
        ...
        for self.ts, self.buf in self.pcap:
            if self.count <= self.dnsPktCount:
                ...
                if self.dns.rcode != dpkt.dns.DNS_RCODE_NOERR:
                    ...
                    if self.dns.rcode == dpkt.dns.DNS_RCODE_NXDOMAIN or self.dns.rcode == dpkt.dns.DNS_RCODE_REFUSED or self.dns.rcode == dpkt.dns.DNS_RCODE_SERVFAIL:
                        self.dnsFailedCount += 1
                        for answer in self.dns.qd:
                            try:
                                dstIP = struct.unpack('!L', self.ip.dst)[0]
                                if self.mode > 0:
                                    self.update_response(self.dns.id, self.inet_ntoa_wrap(self.ip.dst), self.answer.name, self.answer.type, self.dns.rcode, 0, 0, str(datetime.datetime.utcfromtimestamp(self.ts).strftime('%d/%m/%y %H:%M:%S')))
                                if self.mode < 2:
                                    print (
                                     self.dnsPktCount, '\t', self.dns.id, '\tFail/Refused\t', self.answer.name, self.dns.rcode,
                                     self.inet_ntoa_wrap(self.ip.dst), '\t',
                                     str(datetime.datetime.utcfromtimestamp(self.ts).strftime('%H:%M:%S')))
                                continue
                            except:
