...
import sys, socket, datetime, subprocess, struct, time, os.path
...
class Network():
    ...
    def find_anomaly(self):
        ...
        host_cnt = self.UFID
        tmp_str = ''
        tmp_str2 = 'S.No, Hostname, Req_cnt, Dist_Req_Cnt, high_req_cnt_single_domain, avg_req_per_min, high_req_per_min, cnt_query_a, cnt_query_mx, cnt_query_ns, cnt_query_ptr, dist_tld, dist_sld, dist_dns_server, res_cnt,dist_city_cnt, dist_subdivision_cnt, dist_country_cnt, res_rec_cnt, res_success_cnt, res_failed, avg_ttl_value, high_ttl_value, res_ip_cnt\n'
        for m_hosts in self.hosts:
            req_cnt = 0
            dist_req_cnt = 0
        ...
        if os.path.exists(csv_file_name):
            csv_outfile = open(csv_file_name, 'a')
        else:
        ...
        for m_hosts in self.hosts:
            ...
            for m_urls in obj.domain:
                ...
                for token in token_list.list:
                    token_cnt += 1
                    request = token_list.list[token]
                    tmp_str += '<tr><td></td><td></td><td>' + str(token_cnt) + '</td><td >' + request.txn_id + '</td><td >' + request.req_type + '</td><td >' + request.req_timestamp + '</td></tr>'
                    res_cnt = 0
                    for res in request.response:
                        res_cnt += 1
                        tmp_str += '<tr><td></td><td></td><td></td><td>' + str(res_cnt) + '</td><td >' + res.res_code + '</td><td >' + res.ttl + '</td><td >' + res.resolved_ip + '</td><td >' + res.res_timestamp + '</td></tr>'
    ...
    def start_console(self):
        syntax = ' h - display help \n show - list suspect hosts \n savep - Save response packets \n savea - Save all packets \n plot - draw chart \n q - quit'
        print syntax
        print 'console>'
        choice = ''
        while choice != 'q':
            ...
            if choice == 'show':
                result = []
                self.get_top_hosts(100, result)
                i = 1
                print 'Hosts with over 500 distinct requests '
